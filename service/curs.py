...
class Read_curs:
    def __init__(self, date_cred, curr_code):
        ...
        try:
            ...
            # Opening JSON file
            f = open(file='./database/settings_curs_nbu.json',
            ...
            if self.data_format == 'json':
                ...
                self.char_format_date = (
                    data)['main']['curs_nbu_xml']['char_format_date']
            else:
                self.text_error = ("File 'settings_curs_nbu.json' -> parameter "
                                   "'data_format' not in 'xml' or 'json'")
                print(self.text_error)
                return
                    ...
                    if self.data_format == 'json':
                        ...
                        for json_line in JSON_object:
                            params = (date_cred.strftime("%Y-%m-%d"),
                                      ...
                                      1)
                            cursor.execute(
                                "INSERT OR IGNORE INTO CURRENCY(curr_code, curr_name) "
                                "VALUES(?, ?)",
                                (json_line[self.char_curr_code],
                                 json_line[self.char_curr_name]))
                            cursor.execute(
